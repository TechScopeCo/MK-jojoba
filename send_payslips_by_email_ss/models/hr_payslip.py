...
class HRPayslip(models.Model):
    ...
    def payslip_send_mail(self):
        ...
        try:
            template_id = int(
                self.env['ir.config_parameter'].sudo().get_param('send_payslips_by_email_ss.choose_mail_template'))
        except ValueError:
            template_id = False
        if not template_id:
            raise UserError(_("Email Template must be selected in settings."))
        template = template_id and self.env['mail.template'].browse(template_id)
        pdf_bin, format = self.env['ir.actions.report']._render_qweb_pdf('hr_payroll.report_payslip_lang', res_ids=self.ids)
    ...
    def send_payslip_by_email_action(self):
        for rec in self:
            try:
                template_id = int(
                    rec.env['ir.config_parameter'].sudo().get_param('send_payslips_by_email_ss.choose_mail_template'))
            except ValueError:
                template_id = False
            if not template_id:
                raise UserError(_("Email Template must be selected in settings."))
            template = template_id and rec.env['mail.template'].browse(template_id)
            pdf_bin, format = self.env['ir.actions.report']._render_qweb_pdf('hr_payroll.report_payslip_lang',res_ids=rec.ids)
...
class ResConfigSettings(models.TransientModel):
    ...
    choose_mail_template = fields.Many2one('mail.template', string='Mail Template For Payroll',
                                           config_parameter='send_payslips_by_email_ss.choose_mail_template')

    choose_mail_template_for_employee = fields.Many2one('mail.template',
